...
impl Warpainter
{
    ...
    fn flatten(&mut self) -> &Image<4>
    {
        if self.get_temp_edit_image()
        {
            ...
            self.layers.flatten_as_root(self.canvas_width, self.canvas_height, Some(self.current_layer), Some(&self.editing_image_display.as_ref().unwrap()))
        }
        else
        {
    ...
    fn get_temp_edit_image(&mut self) -> bool // only used in flattening
    {
        if let Some(edit_image) = &self.editing_image
        {
            if let Some(layer) = self.layers.find_layer(self.current_layer)
            {
                if !layer.locked
                {
                    ...
                    {
                        //let rect = layer.edited_dirty_rect.unwrap_or([[0.0, 0.0], [0.0, 0.0]]);
                        let rect = self.cache_rect;
        ...
        if self.get_temp_edit_image()
        {
            ...
            if let Some(layer) = self.layers.find_layer_mut(self.current_layer)
            {
                ...
                if !layer.locked
                {
                    ...
                    {
                        std::mem::swap(image, &mut *current_image);
                        ...
                        println!("Edit analysis time: {:.3}ms", start.elapsed().as_secs_f64() * 1000.0);
                        //println!("{}", event.len());
                        let start = web_time::Instant::now();
                        self.undo_buffer.push(event.compress());
                        println!("Edit encoding time: {:.3}ms", start.elapsed().as_secs_f64() * 1000.0);
                    }
                }
            }
        }
...
fn show_modal_warning(ctx : &egui::Context, text : String)
{
    let is_open = ctx.data_mut(|d| d.get_persisted::<bool>(egui::Id::new("modal_warning_state"))).unwrap_or(true);
    ...
    egui::Window::new("Warning").collapsible(false).resizable(false)
        .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0]).title_bar(true).interactable(true)
        .enabled(true).id(egui::Id::new("modal_warning")).show(ctx, |ui|
        {
            ui.label(text);
            if ui.button("Close").clicked()
            {
                ui.memory_mut(|mem|
                {
                    mem.data.insert_persisted(egui::Id::new("modal_warning_state"), false);
                });
            }
        });
}
...
impl eframe::App for Warpainter
{
    fn update(&mut self, ctx : &egui::Context, frame : &mut eframe::Frame)
    {
        ...
        if let Some(gl) = unsafe { &* &raw const GL }
        {
            self.max_texture_size = unsafe { gl.get_parameter_i32(glow::MAX_TEXTURE_SIZE) as i32 };
        }
        ...
        {
            sizewarn = format!(
"Image data (canvas or layer) is larger than this device's maximum supported texture resolution, which is {}.
The affected data may fail to render. If this limit is small, try upgrading your graphics drivers.
This warning will only be shown once.", self.max_texture_size);
        }
        #[cfg(target_arch = "wasm32")]
        {
            sizewarn = format!(
"Image data (canvas or layer) is larger than this browser or device's maximum supported texture resolution, which is {}.
The affected data may fail to render. If this limit is small, try using a different browser or device.
This warning will only be shown once.", self.max_texture_size);
        };
        ...
        self.layers.visit_layers_mut(0, &mut |l, _depth|
        {
            ...
            {
                if data.width > self.max_texture_size as usize || data.height > self.max_texture_size as usize
                {
                    show_modal_warning(ctx, sizewarn.clone());
                    return None;
                }
        ...
        egui::TopBottomPanel::top("ToolBar").show(ctx, |ui|
        {
            egui::menu::bar(ui, |ui|
            {
                ...
                //ui.text_edit_singleline(&mut self.dummy_text);
            });
        });
        
        if check_ui_keyboard_focus(ctx)
        {
            focus_is_global = false;
        }
                ...
                ui.with_layout(egui::Layout::left_to_right(egui::Align::TOP).with_main_wrap(true), |ui|
                {
                    ...
                    if add_button!(ui, "funny alpha", "Funny Alpha Flag", funny_flag).clicked()
                    {
                        if let Some(layer) = self.layers.find_layer_mut(self.current_layer)
                        {
                            ...
                            {
                                let start = web_time::Instant::now();
                                let _n = data.opaque_bounds();
                                println!("outline calc time: {:.3}ms", start.elapsed().as_secs_f64() * 1000.0);
                            }
                        }
                    }
                    */
        ...
        macro_rules! toolsettings { () => { |ui|
        {
            ui.with_layout(egui::Layout::bottom_up(egui::Align::LEFT), |ui|
            {
                ...
                {
                    ui.add(egui::Label::new(egui::RichText::new(&rgbainfotext).size(7.0)).selectable(false)).clicked();
...
fn check_ui_keyboard_focus(ctx : &egui::Context) -> bool
{
    ctx.memory(|mem| mem.focused()).is_some()
}

#[cfg(target_os = "android")]
fn get_insets(app : egui_winit::winit::platform::android::activity::AndroidApp) -> (f32, f32)
{
    ...
    let parent = env.call_method(activity, "getWindow", "()Landroid/view/Window;", &[]).unwrap().l().unwrap();
    let parent = env.call_method(parent, "getDecorView", "()Landroid/view/View;", &[]).unwrap().l().unwrap();
    //println!("{:?}", parent);
    let parent = env.call_method(parent, "getRootWindowInsets", "()Landroid/view/WindowInsets;", &[]).unwrap().l().unwrap();
    //println!("{:?}", parent);

    let type_class = env.find_class("android/view/WindowInsets$Type").unwrap();
    let system_bars_type = env.call_static_method(type_class, "systemBars", "()I", &[]).unwrap().i().unwrap();
    
    //let parent = env.call_method(parent, "getInsets", "(I)Landroid/graphics/Insets;", &[JValue::Int(system_bars_type)]).unwrap().l().unwrap();
    let parent = env.call_method(parent, "getInsets", "(I)Landroid/graphics/Insets;", &[JValue::Int(system_bars_type)]).unwrap().l().unwrap();
    //println!("{:?}", parent);
    let top = env.get_field(&parent, "top", "I").unwrap().i().unwrap();
    let bottom = env.get_field(parent, "bottom", "I").unwrap().i().unwrap();
...
fn android_main(app : egui_winit::winit::platform::android::activity::AndroidApp)
{
    ...
    eframe::run_native(
        "Warpainter",
        options,
        Box::new(|_cc| Ok(Box::<Warpainter>::default())),
    ).unwrap();
}
#[cfg(target_os = "android")]
